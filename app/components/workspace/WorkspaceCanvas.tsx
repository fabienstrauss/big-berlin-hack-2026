...
import { Globe, ImageIcon, Library, Paperclip, Plus, Sparkles, Video } from 'lucide-react';
import {
  Background,
  BackgroundVariant,
  Controls,
...
export function WorkspaceCanvas() {
  ...
  const brainstormActions = [
    { testId: 'toolbar-add-note', label: 'Add note', icon: Plus, onClick: actions.addQuickNote, primary: true },
    { testId: 'toolbar-open-upload', label: 'Own content', icon: Paperclip, onClick: () => setActivePanel('upload') },
    { testId: 'toolbar-open-generate', label: 'Generate', icon: Sparkles, onClick: () => setActivePanel('generate') },
    { testId: 'toolbar-open-scrape', label: 'Tavily scrape', icon: Globe, onClick: () => setActivePanel('scrape') },
    { testId: 'toolbar-open-template', label: 'Templates', icon: Library, onClick: () => setActivePanel('template') },
  ];

  const choiceActions = [
    { testId: 'toolbar-choice-ai-assistant', label: 'AI Assistant', icon: Sparkles, onClick: () => {}, primary: true },
    { testId: 'toolbar-choice-saved-presets', label: 'Saved Presets', icon: Library, onClick: () => {} },
  ];
  ...
  return (
    <div className="relative h-[100dvh] overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(240,249,255,0.95),_rgba(248,250,252,1)_45%,_rgba(255,255,255,1)_100%)] text-slate-900">
      ...
      <main className="relative h-full overflow-hidden">
        ...
        >
          <ChoiceScreen
            key={persistedChoiceConfig ? 'choice-config-hydrated' : 'choice-config-empty'}
